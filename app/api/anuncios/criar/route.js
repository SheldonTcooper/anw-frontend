...
export async function POST(request) {
  try {
    ...
    console.log("fotosUrls recebidos:", fotosUrls?.length || 0)
    ...
    if (!usuario) {
      usuario = await prisma.usuarios.create({ data: { id: gerarId(), email, senhaHash: 'temp', nome: titulo, tipo: 'ANUNCIANTE', atualizadoEm: new Date() } })
    }

    const anuncio = await prisma.anuncios.create({
      data: { id: gerarId(), usuarioId: usuario.id, titulo, slug, descricao, cidade, estado: estado || 'BR', bairro, whatsapp: whatsapp?.replace(/\D/g, ''), cache: cache ? parseFloat(cache) : null, dataNascimento: dataNascimento ? new Date(dataNascimento) : null, plano: 'ORGANICO', status: 'EM_ANALISE', atualizadoEm: new Date() }
    })
    ...
    if (fotosUrls && fotosUrls.length > 0) {
      for (let i = 0; i < fotosUrls.length; i++) {
        ...
        await prisma.midias.create({
          data: { id: gerarId(), anuncioId: anuncio.id, tipo: "FOTO", url: fotosUrls[i], ordem: i }
        })
      }
      console.log("Fotos salvas:", fotosUrls.length)
    }
