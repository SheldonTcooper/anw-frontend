...
const schema = z.object({
  ...
  servicos:       z.array(z.string()).max(50).optional(),
  biotipos:       z.array(z.string()).max(10).optional(),
})
...
export async function POST(request) {
  ...
  if (!limit.ok) {
    return Response.json({ success: false, error: "Muitas tentativas. Aguarde " + limit.espera + " segundos." }, { status: 429 })
  }
  ...
  try {
    ...
    if (!usuario) {
      usuario = await prisma.usuarios.create({
        data: { id: gerarId(), email, senhaHash: 'temp', nome: titulo, tipo: 'ANUNCIANTE', atualizadoEm: new Date() }
      })
    }

    const anuncio = await prisma.anuncios.create({
      data: {
        id: gerarId(), usuarioId: usuario.id, titulo, slug, descricao, cidade,
        estado, bairro, whatsapp: whatsappLimpo,
        cache: cache ? parseFloat(String(cache)) : null,
        dataNascimento: dataNascimento ? new Date(dataNascimento) : null,
        plano: 'ORGANICO', status: 'EM_ANALISE', atualizadoEm: new Date()
      }
    })
    ...
    if (fotosUrls && fotosUrls.length > 0) {
      for (let i = 0; i < fotosUrls.length; i++) {
        await prisma.midias.create({
          data: { id: gerarId(), anuncioId: anuncio.id, tipo: "FOTO", url: fotosUrls[i], ordem: i, aprovada: true, isCapa: i === 0 }
        })
      }
    }

    // Salva servicos e biotipos como tags
    const todasTags = [
    ...
    for (const tagData of todasTags) {
      let tag = await prisma.tags.findUnique({ where: { slug: tagData.slug } })
      if (!tag) {
        tag = await prisma.tags.create({
          data: { id: gerarId(), slug: tagData.slug, nome: tagData.nome, cluster: tagData.cluster }
        })
      }
      await prisma.anuncios_tags.create({
        data: { anuncioId: anuncio.id, tagId: tag.id }
      }).catch(() => {})
    }
