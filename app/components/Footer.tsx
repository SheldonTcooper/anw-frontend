...
const links = [
  ...
  { label: "Suporte", href: "https://wa.me/5585991879866", externo: true },
...
export default function Footer() {
  return (
    <footer className="w-full px-4 py-8 sm:px-8" style={{ backgroundColor: "#250C30", borderTop: "1px solid #4A1A5C" }}>
      <div className="mx-auto max-w-6xl flex flex-col gap-6">
        <div className="flex flex-col items-center gap-4 sm:flex-row sm:justify-between">
          <span className="text-base font-bold" style={{ color: "#C0306A" }}>AcompanhantesNaWeb</span>
          <nav className="flex flex-wrap justify-center gap-x-5 gap-y-2">
            {links.map(({ label, href, externo }) => (
              <a key={label} href={href} target={externo ? "_blank" : undefined} rel={externo ? "noopener noreferrer" : undefined}
                className="text-sm transition-colors hover:text-white" style={{ color: "#c9a8e0" }}>
                {label}
              </a>
            ))}
          </nav>
